...
<form action="ManagerServlet_sadmin" method="get" id="queryForm">
	  <input type="hidden" name="method" value="findTime">
		 ...
		 <div class="d">
			  <center>
			      <c:forEach items= "${requestScope.page.recordList }" var="user" varStatus="status">
					   <div style="color: #242262;width: 220px;height: 280px;border-radius: 10px;background: #FFFFFF;float: left;margin-left:30px ;margin-top: 80px">
                       <br>
                       <center>
                         <table>
                        	<tr><td>用户名：</td><td>${user.username }</td></tr>
                       		...
                       		<tr><td>要求学时：</td><td>${user.ltime }</td></tr>
                       		<tr><td>实际学时：</td><td>${user.utime }</td></tr>
						 </table>
                        </center>  
                        </div>
                   </c:forEach>
